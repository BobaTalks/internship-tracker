...
def get_facebook_internships(soup):
    # print(soup)
    internship_titles = [container for container in soup.select(".x1n2onr6")]
    internship_locations = [
        container.get("data-tooltip-content")
        for container in soup.select(
            "._8seh div:first-child div:first-child div:first-child"
        )
    ]
    internship_links = [
        "https://www.metacareers.com" + container.get("href")
        for container in soup.select("._af0h a:first-child")
    ]
    print(internship_titles)
    print(len(internship_titles))
    print(len(internship_links))
    internships = [
...
facebook_internships = scrape_internships(urls["Facebook"], "Facebook")
apple_internships = scrape_internships(urls["Apple"], "Apple")

# print(facebook_internships)
